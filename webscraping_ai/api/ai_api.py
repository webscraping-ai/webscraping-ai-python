...
class AIApi:
    ...
    def get_question(
        ...
        context_limit: Annotated[Optional[StrictInt], Field(description="Maximum number of tokens to use as context for the LLM model (4000 by default).")] = None,
        response_tokens: Annotated[Optional[StrictInt], Field(description="Maximum number of tokens to return in the LLM model response. The total context size (context_limit) includes the question, the target page content and the response, so this parameter reserves tokens for the response (see also on_context_limit).")] = None,
        on_context_limit: Annotated[Optional[StrictStr], Field(description="What to do if the context_limit parameter is exceeded (truncate by default). The context is exceeded when the target page content is too long.")] = None,
        ...
        :type question: str
        :param context_limit: Maximum number of tokens to use as context for the LLM model (4000 by default).
        :type context_limit: int
        :param response_tokens: Maximum number of tokens to return in the LLM model response. The total context size (context_limit) includes the question, the target page content and the response, so this parameter reserves tokens for the response (see also on_context_limit).
        :type response_tokens: int
        :param on_context_limit: What to do if the context_limit parameter is exceeded (truncate by default). The context is exceeded when the target page content is too long.
        :type on_context_limit: str
    ...
    def _get_question_serialize(
        ...
        if context_limit is not None:
            ...
            _query_params.append(('context_limit', context_limit))
            
        if response_tokens is not None:
            
            _query_params.append(('response_tokens', response_tokens))
            
        if on_context_limit is not None:
            
            _query_params.append(('on_context_limit', on_context_limit))
